    ...
    <div class="col-12 d-flex no-block align-items-center">
      ...
      <div class="ms-auto text-end">
        ...
        @else
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="{{ route('appointment.cancelled') }}" class="btn btn-info">Cancelled Appointment</a></li>
            </ol>
          </nav>
        @endif
      </div>
              ...
              <tbody>
                ...
                @php $i = 0; @endphp
                  @foreach($appointments as $key =>  $a)
                    @if($user->role_id == 1)
                      @if($a->status != 1 && ($a->payment_id  != NULL || $a->patient->type_of_patient == 1))
                        <tr>
                          <td>{{$i+1}}</td>
                          <td>@if($a->patient != NULL){{$a->patient->first_name}} {{$a->patient->last_name}} @else NA @endif</td>
                          <td>@if($a->patient != NULL) {{$a->patient->uhid}} @endif </td>
                          ...
                          <td>
                            ...
                            @if($a->status != 1)<a href="{{route('appointment.more', $a->id)}}" class="fas fa-prescription fa-2x text-info" title="Prescription"></a> &nbsp;
                              ...
                              @endif
                              @if($user->role_id == 1 && date('Y-m-d') == date('Y-m-d',strtotime($a->schedule_date)) && $a->type == 'online')
                                <a href="javascript:void(0);" class="fa fa-video fa-2x" title="Video Call" onClick="videoCall({{$a->id}})"></a>
                              @endif
                            @else
